...
// If you find a reference that is a parent of the current chain then you need to return a cascading failure, you can't
// generate an example for this for infinite recursion. If it's not required, just ignore the field.

class ChainContext {
  private _resolvedReferences: Set<string>;
  private _lookup: Lookup;
  private _depth: number;
  private _stage: Stage;
  private _parent: JsonSchema | undefined;
  ...
  constructor(
    ...
    parent: JsonSchema | undefined) {
      this._resolvedReferences = resolvedReferences;
      this._lookup = lookup;
      this._depth = depth;
      this._stage = stage;
      this._parent = parent;
  ...
  public registerReference(ref: NonNullable<JsonSchema1['$ref']>) {
    this._resolvedReferences.add(ref);
  }

  public seenBefore(ref: NonNullable<JsonSchema1['$ref']>): boolean {
    return this._resolvedReferences.has(ref);
  }

  public clone(currentParent: JsonSchema): ChainContext {
    return new ChainContext(new Set(this._resolvedReferences), this._lookup, this._depth + 1, this._stage, currentParent);
  }
}
...
function missingSchema(schemaOrRef: JsonSchema): Error {
  return new Error('missing-schema', `Could not find a schema for: ${JSON.stringify(schemaOrRef)}`);
}
...
function infinitePropLoopForObject(propName: string, ref: NonNullable<JsonSchema1['$ref']>, schema: JsonSchema1): Error {
  return new Error(
    'infinite-prop-loop',
    `The reference to '${ref}' in the property '${propName}' in the schema '${schema.title || 'object'}'
...
function allOfMismatchedTypes(allTypes: (string | null)[]): Error {
  return new Error(
    'all-of-mismatched-types',
    `There was an allOf that evaluated to examples of mismatched types: ${JSON.stringify(allTypes)}`
  );
}

function nothing(parentSchema: JsonSchema | undefined): Error {
  const renderedParent = parentSchema === undefined ? 'root' : JSON.stringify(parentSchema);
  return new Error('example-of-nothing-is-impossible', `Can't generate an example of the 'nothing' type for a child of: ${renderedParent}`);
}
...
function isNameAndExample(t: NameAndExample | IgnoredProperty): t is NameAndExample {
  return 'example' in t;
}

function inferExample<A>(schema: JsonSchema1, typeMatcher: (x: any) => boolean, defaultExample: () => Example): Example {
  const match = (schema.examples || []).find(typeMatcher);
  if (match !== undefined) {
    return Example.of(match);
  }

  if (schema.enum !== undefined && schema.enum.length > 0 && typeMatcher(schema.enum[0])) {
  ...
  if (type !== undefined) {
    if (type === 'boolean') {
      ...
      if (itemSchema === undefined) {
        ...
        const itemExample = generateJsonExampleForHelper(nextContext, itemSchema);
      ...
      if (properties === undefined) {
        ...
        const props = Object.keys(properties)
          ...
          .map<NameAndExample | IgnoredProperty>(name => {
            ...
            if (propOrRef.$ref !== undefined) {
              if (context.seenBefore(propOrRef.$ref)) {
                return requiredPropNames.has(name)
                  ? { name, example: Errors.of(infinitePropLoopForObject(name, propOrRef.$ref, schema)) }
                  : { name };
              }
            ...
            const generatedExample = generateJsonExampleForHelper(nextContext, prop);
        ...
        const e = Errors.from(...nonIgnoredProps.map(p => p.example).filter<Errors>(isErrors));
        if (e.length > 0) {
          return e;
        }
    ...
    if (schema.anyOf !== undefined && schema.anyOf.length > 0) {
      ...
      const exs = potentialSchemas.filter(isSchema).map(s => generateJsonExampleForHelper(nextContext, s));
      ...
      const matchedType = allExampleTypes.reduce((a, b) => a === b ? a : null);
      if (matchedType === null) {
      ...
      if (matchedType === 'object') {
        ...
        return Example.of(example);
      } else if (matchedType === 'string' || matchedType === 'number' || matchedType === 'boolean') {
